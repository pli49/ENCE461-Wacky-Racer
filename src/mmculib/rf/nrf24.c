...
static uint8_t nrf24_write_registers(nrf24_t *nrf, uint8_t reg, const uint8_t *buffer,
                                    uint8_t len)
{
    uint8_t status;
...
void nrf24_set_retries(nrf24_t *nrf, uint8_t delay, uint8_t count)
{
    ...
    nrf24_write_register(nrf, SETUP_RETR, (delay & 0xf) << ARD | (count & 0xf) << ARC);
}
